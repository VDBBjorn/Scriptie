\documentclass[11pt,a4paper,oneside,notitlepage]{book}
\usepackage[english,dutch]{babel}

% marges aanpassen
% (opmerking: moet *voor* inclusie van fancyhdr package komen)
\setlength{\hoffset}{-1in}
\setlength{\voffset}{-1in}
\setlength{\topmargin}{2cm}
\setlength{\headheight}{0.5cm}
\setlength{\headsep}{1cm}
\setlength{\oddsidemargin}{3.5cm}
\setlength{\evensidemargin}{3.5cm}
\setlength{\textwidth}{16cm}
\setlength{\textheight}{23.3cm}
\setlength{\footskip}{1.5cm}

\usepackage[a4paper,plainpages=false]{hyperref}    % Om hyperlinks te hebben in het pdfdocument.
\usepackage{url}
\usepackage{fancyhdr}
\usepackage{graphicx}
%custom packages
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{ltcaption}
\usepackage{listings}
\usepackage{pdfpages}
\usepackage{rotating}
\lstset{
	language=C,
	basicstyle=\small\sffamily,
	breaklines=true,
	numberstyle=\tiny,
	frame=tb,
	columns=fullflexible,
	showstringspaces=false
}
\newcommand{\quotes}[1]{``#1''}
% \usepackage[colorlinks]{hyperref}

\pagestyle{fancy}

\renewcommand{\chaptermark}[1]{\markright{\MakeUppercase{#1}}}
\renewcommand{\sectionmark}[1]{\markright{\thesection~#1}}
\newcommand\fnumber{\mathop{\mbox{$F$-$score$}}}

\newcommand{\headerfmt}[1]{\textsl{\textsf{#1}}}
\newcommand{\headerfmtpage}[1]{\textsf{#1}} 

\fancyhf{}
\fancyhead[LE,RO]{\headerfmtpage{\thepage}}
\fancyhead[LO]{\headerfmt{\rightmark}}
\fancyhead[RE]{\headerfmt{\leftmark}}
\renewcommand{\headrulewidth}{0.5pt}
\renewcommand{\footrulewidth}{0pt}


\fancypagestyle{plain}{ % eerste bladzijde van een hoofdstuk
  \fancyhf{}
  \fancyhead[LE,RO]{\headerfmtpage{\thepage}}
  \fancyhead[LO]{\headerfmt{\rightmark}}
  \fancyhead[RE]{\headerfmt{\leftmark}}
  \renewcommand{\headrulewidth}{0.5pt}
  \renewcommand{\footrulewidth}{0pt}
}

\newtheorem{theorem}{Theorem}

\hypersetup{
    pdfauthor = {Bjorn Vandenbussche},
    pdftitle = {Verbeterde aanbevelingssystemen op basis van content recognition in tekst},
    pdfsubject = {Scriptie voorgelegd tot het behalen van de graad van Master of Science in de Industri\"ele Wetenschappen: Informatica, juni 2016}
}

% anderhalve interlinie (opm: titelblad gaat uit van 1.5)
\renewcommand{\baselinestretch}{1.5}

% indien LaTeX niet goed splitst, neem je woord hierin op, of evt om splitsen 
% te voorkomen
\hyphenation{}

\begin{document}

\selectlanguage{dutch}
% titelblad (voor kaft)
%\include{titel}
\includepdf[pages={-},offset=25mm -25mm]{voorblad.pdf}

% lege pagina (!!)
% titelblad (!!)

% geen paginanummering tot we aan de inhoudsopgave komen
\pagestyle{empty}

% voorwoord met dankwoord en toelating tot bruikleen (ondertekend)
\include{voorwoord}

% overzicht
\include{overzicht}
%\addcontentsline{Extended abstract}
\includepdf[pages={-},offset=25mm -25mm]{abstract.pdf}


\pagestyle{fancy}
\frontmatter

% inhoudstafel
\tableofcontents

% opmaak voor het eigenlijke boek; onderstaande lijnen
% weglaten als de eerste regel van een nieuwe alinea moet
% inspringen in plaats van extra tussenruimte
\setlength{\parindent}{0pt}
\setlength{\parskip}{0.5\baselineskip plus 0.5ex minus 0.2ex}
\setlength{\parskip}{1ex plus 0.5ex minus 0.2ex}

% hoofdstukken
\mainmatter

% hier worden de hoofdstukken ingevoegd (\includes)
\include{hoofdstukken/Probleemstelling}
\include{hoofdstukken/Bestaande-systemen}
\include{hoofdstukken/Datasets}
\include{hoofdstukken/Architectuur}
\include{hoofdstukken/Werking}
\include{hoofdstukken/Resultaten}
\include{hoofdstukken/Conclusie}
% appendices
\appendix

% hier worden de appendices ingevoegd (\includes)
\include{bijlagen/bijlageA}
\include{bijlagen/bijlageB}

\include{referenties}

\backmatter

% eventueel: lijst van figuren en tabellen
\listoffigures
\listoftables

% lege pagina (!!)


% kaft

\end{document}
\grid
